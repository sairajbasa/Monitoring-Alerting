...
- ð¥ Total CPU usage (% per instance)
```
100 - (avg by(instance) (rate(node_cpu_seconds_total{mode="idle"}[5m])) * 100)
```
-â¡ CPU usage per core
```
100 - (rate(node_cpu_seconds_total{mode="idle"}[5m]) * 100)
```
- ð Good for detailed graphs

- ð Top 5 high CPU servers
- ð§  Memory
```
...
```
- ð¸ Network Traffic (Incoming)
```
rate(node_network_receive_bytes_total[5m])
```
- ð¸ Network Traffic (Outgoing)
```
